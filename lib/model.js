...
model.prototype.valid = function(data) {
	const self = this;
	return new Promise(function(resolve, reject) {
		const valid = self._validate(data);
		if (!valid)
...
model.prototype.beforeUpdateValidate = function(data, useFilterUpdate = true) {
	...
	return self.valid(toUpdate).then(data => {
		if (self._addTimeStamps) {
			data.updatedAt = new Date();
		}
		return data;
...
model.prototype.createIndexes = async function(query,projection) {
	const self = this;
...
model.prototype.find = function(query,projection) {
	...
	return new find(query,projection,this._getDb);
}

model.prototype.count = function(query) {
	const self = this;
	const db= self._getDb.getDb();
	const Model= db.collection(self._modelName);
...
model.prototype.insertMany = async function(data,findDocs=true) {
	...
	if(self.beforeWrite)
		data= data.map(self.beforeWrite);
...
model.prototype.updateOne = async function(query,data,options={},findDocs=true) {
	...
	{
		if(self.beforeWrite)
			data.$set= self.beforeWrite(data.$set);
...
model.prototype.updateMany = async function(query,data,options={},findDocs=true) {
	...
	{
		if(self.beforeWrite)
			data.$set= data.$set.map(self.beforeWrite);
